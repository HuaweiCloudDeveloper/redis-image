...
module "ecs-security-group" {
  ...
  secgroup_rules_configuration = [
    ...
    { description      = null, direction = "ingress", ethertype = "IPv4", protocol = "tcp", ports = "6379",
      remote_ip_prefix = "0.0.0.0/0", remote_group_id = null
    },     
...
module "ecs" {
  ...
  instance_image_id  = var.Version == "v6.2.7" ? local.instance_image_id_maps_v627[data.huaweicloud_availability_zones.az.region] : local.instance_image_id_maps_v627[data.huaweicloud_availability_zones.az.region] 
